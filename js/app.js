...
***************************************************************************** */
    ...
    da.init();
    window.onload = event => {
        if (document.querySelector("#myMap")) map();
    };
    function map(n) {
        ymaps.ready(init);
        function init() {
            var myMap = new ymaps.Map("myMap", {
                controls: [ "trafficControl", "typeSelector", "fullscreenControl", "zoomControl", "geolocationControl" ],
                center: [ 55.76, 37.64 ],
                zoom: 11
            });
            var polygon = new ymaps.Polygon([ [ [ 55.9378797392184, 37.59188059765626 ], [ 55.91937623050167, 37.852805890625014 ], [ 55.79733984395754, 37.959922589843764 ], [ 55.62677390279352, 38.0011213203125 ], [ 55.53498013691004, 37.92147044140625 ], [ 55.51004974710499, 37.72371653515627 ], [ 55.50849107094701, 37.572654523437514 ], [ 55.55989466465658, 37.43807200390625 ], [ 55.690435916574906, 37.273277082031264 ], [ 55.8375462190853, 37.259544171875014 ], [ 55.91937623050167, 37.36391428906249 ] ] ], {
    ...
    const tabButton = document.querySelectorAll("[data-goal-subject]");
    for (let i = 0; i < goalSelectRel.length; i++) Array.from(goalSelectRel[i].children).forEach((function(el) {
        el.addEventListener("click", goalChange);
        el.addEventListener("click", goalTabChange);
    }));
    Array.from(tabButton).forEach((function(el) {
        el.addEventListener("click", filterItems);
        el.addEventListener("click", goalTabChange);
        el.addEventListener("click", goalChange);
        el.addEventListener("click", orderTitleChange);
    }));
    function goalChange() {
        if (1 == this.dataset.goal || 1 == this.dataset.goalSubject) {
            for (let k = 0; k < goalSelect.length; k++) goalSelect[k].classList.remove("_filter-active");
            for (let n = 0; n < goalSelectRel.length; n++) goalSelectRel[n].children[0].classList.add("_filter-active");
            return;
        }
        if (2 == this.dataset.goal || 2 == this.dataset.goalSubject) {
            for (let k = 0; k < goalSelect.length; k++) goalSelect[k].classList.remove("_filter-active");
            for (let n = 0; n < goalSelectRel.length; n++) goalSelectRel[n].children[1].classList.add("_filter-active");
            return;
        }
        if (3 == this.dataset.goal || 3 == this.dataset.goalSubject) {
            for (let k = 0; k < goalSelect.length; k++) goalSelect[k].classList.remove("_filter-active");
            for (let n = 0; n < goalSelectRel.length; n++) goalSelectRel[n].children[2].classList.add("_filter-active");
            return;
        }
    }
    function goalTabChange() {
        if (1 == this.dataset.goal || 1 == this.dataset.goalSubject) {
            for (let i = 0; i < tabButton.length; i++) tabButton[i].classList.add("_tab-disabled");
            for (let i = 0; i < 3; i++) tabButton[i].classList.remove("_tab-disabled");
            return;
        }
        if (2 == this.dataset.goal || 2 == this.dataset.goalSubject) {
            for (let i = 0; i < tabButton.length; i++) tabButton[i].classList.add("_tab-disabled");
            for (let i = 3; i < 4; i++) tabButton[i].classList.remove("_tab-disabled");
            return;
        }
        if (3 == this.dataset.goal || 3 == this.dataset.goalSubject) {
            for (let i = 0; i < tabButton.length; i++) tabButton[i].classList.add("_tab-disabled");
            for (let i = 4; i < 6; i++) tabButton[i].classList.remove("_tab-disabled");
            return;
        }
    }
    const filterButtons = document.querySelectorAll("[data-short-name]");
    const filterButtonsContent = {
        mobile: [ "ПН", "ВТ", "СР", "ЧТ", "ПТ", "СБ", "ВС" ],
        desctop: [ "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота", "Воскресенье" ]
    };
    const orderRadio = document.querySelectorAll(".total-foodcalc__order-option");
    const orderTitle = document.querySelector(".total-foodcalc__order-title");
    const orderTitleOptions = [ "База 600 ККАЛ", "Похудение 800 ККАЛ", "Похудение 1100 ККАЛ", "Поддержание 1500 ККАЛ", "Набор 2000 ККАЛ", "Набор 2500 ККАЛ" ];
    function orderTitleChange() {
        for (let i = 0; i < tabButton.length; i++) if (this == tabButton[i]) {
            orderTitle.innerHTML = orderTitleOptions[i];
            orderRadio[i].click();
        }
    }
    function filterButtonsChange() {
        if (filterButtons) for (let i = 0; i < filterButtons.length; i++) if (window.innerWidth < 768) filterButtons[i].innerHTML = filterButtonsContent.mobile[i]; else filterButtons[i].innerHTML = filterButtonsContent.desctop[i];
    }
    filterButtonsChange();
    window.addEventListener("resize", filterButtonsChange, false);
    const productContainer = document.querySelector(".schedule-foodcalc__items");
    const productItems = document.querySelector(".schedule-foodcalc__items").children;
    const dayFilterButtons = document.querySelectorAll("[data-filter-day]");
    const typeFilterButtons = document.querySelectorAll("[data-filter-type]");
    const caloryCalc = document.querySelectorAll(".goal-schedule__info>.info-schedule__subtitle>span");
    let productItemsArr = [];
    function productFilterInit() {
        for (let i = 0; i < productItems.length; i++) productItemsArr.push(productItems[i]);
    ...
    function declOfNum(n, text_forms) {
        ...
        if (1 == n1) return text_forms[0];
        return text_forms[2];
    }
    function filterItems() {
        productContainer.innerHTML = "";
        for (let i = 0; i < tabButton.length; i++) {
            if (tabButton[i].classList.contains("_tab-active")) for (let j = 0; j < dayFilterButtons.length; j++) if (dayFilterButtons[j].checked) for (let k = 0; k < typeFilterButtons.length; k++) if (typeFilterButtons[k].checked) {
                let productItemsArrTemp = [];
                for (let item = 0; item < productItemsArr.length; item++) if (productItemsArr[item].dataset.categoryPreset == tabButton[i].dataset.filterPreset && productItemsArr[item].dataset.categoryDay == dayFilterButtons[j].dataset.filterDay && productItemsArr[item].dataset.categoryType == typeFilterButtons[k].dataset.filterType) {
                    productContainer.innerHTML += productItemsArr[item].outerHTML;
                    productItemsArrTemp.push(productItemsArr[item]);
                    let sumCalories = 0;
                    let sumProteins = 0;
                    let sumFats = 0;
                    let sumCarbs = 0;
                    for (let n = 0; n < productItemsArrTemp.length; n++) {
                        sumCalories += parseInt(productItemsArrTemp[n].dataset.totalCalories);
                        sumProteins += parseInt(productItemsArrTemp[n].dataset.totalProteins);
                        sumFats += parseInt(productItemsArrTemp[n].dataset.totalFats);
                        sumCarbs += parseInt(productItemsArrTemp[n].dataset.totalCarbs);
                        caloryCalc[1].innerHTML = sumCalories;
                        caloryCalc[2].innerHTML = sumProteins;
                        caloryCalc[3].innerHTML = sumFats;
                        caloryCalc[4].innerHTML = sumCarbs;
                    }
                    caloryCalc[0].innerHTML = productItemsArrTemp.length + " " + declOfNum(productItemsArrTemp.length, [ "БЛЮДО", "БЛЮДА", "БЛЮД" ]);
                }
            }
            if (this) this.click();
        }
    }
    const selectForm = document.querySelectorAll("[data-select-preset]");
    const selectBody = document.querySelectorAll(".select_order-select");
    function filterOrder() {
        for (let i = 0; i < tabButton.length; i++) {
            if (tabButton[i].classList.contains("_tab-active")) for (let j = 0; j < typeFilterButtons.length; j++) if (typeFilterButtons[j].checked) for (let sel = 0; sel < selectBody.length; sel++) {
                if (selectBody) {
                    selectBody[sel].style.display = "none";
                    selectForm[sel].setAttribute("disabled", "");
                }
                if (selectForm[sel].dataset.selectPreset == tabButton[i].dataset.filterPreset && selectForm[sel].dataset.selectCategory == typeFilterButtons[j].dataset.filterType) {
                    selectBody[sel].style.display = "block";
                    selectForm[sel].removeAttribute("disabled", "");
                    let activeSelect = selectBody[sel].children[1].children[1].children[0].children;
                    let selectItems = selectForm[sel].children;
                    let totalPriceSelect = document.getElementById("priceTotal");
                    let totalPriceLabel = document.querySelector(".total-foodcalc__order-total>label>span");
                    for (let act = 0; act < activeSelect.length; act++) if (activeSelect[act].hasAttribute("hidden")) {
                        totalPriceSelect.value = selectItems[act].dataset.selectPrice;
                        totalPriceLabel.innerHTML = selectItems[act].dataset.selectPrice;
                    }
                    Array.from(activeSelect).forEach((function(el) {
                        el.addEventListener("click", filterOrder);
                    }));
                }
            }
            if (this) this.click();
    ...
    Array.from(dayFilterButtons).forEach((function(el) {
        el.addEventListener("click", filterOrder);
    }));
    Array.from(typeFilterButtons).forEach((function(el) {
        el.addEventListener("click", filterOrder);
    }));
    Array.from(dayFilterButtons).forEach((function(el) {
        el.addEventListener("click", filterItems);
